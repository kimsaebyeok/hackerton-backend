...
/**
 * OpenAI 호환 Chat Completions 클라이언트 (API key 방식).
 * GLM(Zhipu, base-url …/paas/v4) · OpenAI(base-url …/v1) 등 호환 엔드포인트를 base-url + model 로 전환.
 * 경로는 base-url 에 버전 세그먼트까지 포함하고 여기서 "/chat/completions" 를 붙인다.
 */
@Component
...
public class OpenAiCompatibleLlmClient implements LlmClient {
    ...
    // ── OpenAI 호환 Chat Completions 와이어 포맷 ─────────────────
